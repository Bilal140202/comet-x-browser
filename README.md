...
| Multi-model | Groq, OpenRouter, **NVIDIA NIM (v2.2.0)**, Hugging Face router, any OpenAI-compatible endpoint — one provider abstraction, live model discovery, capability negotiation, AUTO model selection |
| Cloud AI center (v2.2.0) | a Jetpack Compose / Material 3 Expressive screen: paste your personal API keys (NVIDIA `nvapi-…`, OpenRouter `sk-or-v1-…`, Groq, HF, custom), get a live **Test connection** (real catalog fetch + tiny completion), reorder the fallback chain — base URLs and model configs pre-verified against the live services. Keys are Keystore-encrypted, masked on screen, never logged |
| Discord push (v2.2.0) | optional: your own Discord bot posts one result embed to your channel when a background task completes or fails (REST-only, verified live; the bot never reads messages) |
...
2. Open **Settings → AI Provider**, paste an API key, press **Test & Enable**. That's it: Comet-X discovers the provider's live model catalog, checks what each model supports (JSON / tools / vision), picks the best agent-compatible model automatically (**AUTO**) and is ready. Prefer the modern UI? **Settings → Open Cloud AI center** (Material 3 + Compose) does the same with live connection tests and chain reordering — NVIDIA NIM and OpenRouter configs come pre-verified. Prefer zero keys? **Settings → On-device AI** downloads a GGUF model (background, resumable, SHA-256 verified) and the agent runs offline. Want the Transformers.js model zoo instead? **Settings → In-browser AI (Transformers.js)** — pick a model, the browser engine runs it in WebAssembly.
