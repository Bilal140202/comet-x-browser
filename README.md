...
| Multi-model | Groq, OpenRouter, **NVIDIA NIM** (v2.2.0, live-verified), Hugging Face router, any OpenAI-compatible endpoint — one provider abstraction, live model discovery, capability negotiation, AUTO model selection. All BYOK: your key, encrypted with the Android Keystore, sent only to the provider |
| Cloud AI center (v2.3.0) | the app's first **Jetpack Compose / Material 3 Expressive** screen (Settings → Open Cloud AI center): per-provider cards with masked key fields, live **Test connection** (real catalog fetch + tiny completion), AUTO/MANUAL model selection, chain reordering — same encrypted settings keys as the classic UI, so both stay consistent |
...
| Discord agent monitor (v2.2.0) | paste a Discord **bot token + channel ID** and every background task mirrors its story into your channel — start, step milestones, approval gates, the result — so any device with Discord becomes a task monitor. Fire-and-forget: Discord outages can never touch a running task. Bot token Keystore-encrypted |
