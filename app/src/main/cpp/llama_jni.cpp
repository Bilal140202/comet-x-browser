 ...
 *
 * Design (mirrors the JARVIS reference implementation, adapted to Comet-X):
 ...
 *  - Cancellation flag checked between generated tokens / prefill chunks for
 *    immediate stop (AgentEngine.stop() → router.cancelLocal()).
 *  - Returns raw UTF-8 bytes (jbyteArray) to avoid JNI modified-UTF8 limits.
 *  - Progress callback is STRICTLY OPTIONAL: a failed GetMethodID (e.g. under
 *    future minification) can never discard a generation (JARVIS v1.5.0 fix).
 *  - countTokens lets the Kotlin layer enforce the context budget BEFORE
 *    decode and raise ContextTooLargeException (§19 compression ladder).
 */
...
Java_com_cometx_browser_ai_local_LlamaBridge_nativeLoadModel(
    ...
    mparams.n_gpu_layers = 0; // CPU inference: deterministic across devices, no OpenCL/Vulkan driver lottery
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_cometx_browser_ai_local_LlamaBridge_nativeIsLoaded(
        JNIEnv * /*env*/, jobject /*thiz*/) {
    return (g_model != nullptr && g_ctx != nullptr) ? JNI_TRUE : JNI_FALSE;
}

extern "C" JNIEXPORT jint JNICALL
Java_com_cometx_browser_ai_local_LlamaBridge_nativeContextSize(
        JNIEnv * /*env*/, jobject /*thiz*/) {
    return g_configured_ctx;
}

extern "C" JNIEXPORT jint JNICALL
Java_com_cometx_browser_ai_local_LlamaBridge_nativeCountTokens(
        JNIEnv *env, jobject /*thiz*/, jstring text) {
    if (g_model == nullptr) return -1;
    const std::string s = jstring_to_std(env, text);
    if (s.empty()) return 0;
    std::vector<llama_token> tokens(s.size() + 32);
    int n = llama_tokenize(g_model, s.c_str(), static_cast<int32_t>(s.size()),
                           tokens.data(), static_cast<int32_t>(tokens.size()), true, true);
    if (n < 0) {
        tokens.resize(static_cast<size_t>(-n));
        n = llama_tokenize(g_model, s.c_str(), static_cast<int32_t>(s.size()),
                           tokens.data(), static_cast<int32_t>(tokens.size()), true, true);
    }
    return n;
}
...
Java_com_cometx_browser_ai_local_LlamaBridge_nativeCancel(
        JNIEnv * /*env*/, jobject /*thiz*/) {
    g_cancel = true;
}

extern "C" JNIEXPORT void JNICALL
Java_com_cometx_browser_ai_local_LlamaBridge_nativeFree(
        JNIEnv * /*env*/, jobject /*thiz*/) {
    free_all();
    g_configured_ctx = 0;
}
...
Java_com_cometx_browser_ai_local_LlamaBridge_nativeComplete(
        JNIEnv *env, jobject /*thiz*/, jstring prompt, jint max_tokens, jfloat temperature,
        jobject listener, jobjectArray stop_sequences) {
    if (g_model == nullptr || g_ctx == nullptr) return nullptr;
    const std::string text = jstring_to_std(env, prompt);
    if (text.empty()) return nullptr;
    g_cancel = false;

    // Stop sequences: generation halts natively the moment one appears (token /
    // battery savings); the matched marker itself is trimmed off the result.
    // Kotlin layer supplies the chat-template end markers so a small model that
    // echoes template control tokens as text is cut at the boundary instead of
    // burning its whole token budget.
    ...
    // Optional progress listener (same-thread callback; signature onProgress(IIII[B)V:
    // phase 0 = prompt eval, 1 = decoding; partial is UTF-8 bytes so arbitrary model
    // output never hits the modified-UTF8 limit).
    ...
    if (listener != nullptr) {
        ...
        if (cls != nullptr) {
            ...
            if (env->ExceptionCheck()) {
                // Lookup failed: progress is OPTIONAL — clear the pending exception
                // and keep generating. The result must never be thrown away over a
                // UI callback (root cause of the JARVIS v1.5.0 minification bug).
                env->ExceptionClear();
                on_progress = nullptr;
    ...
    // Last-resort guard: trim from the head (keep most recent context) if oversized.
    // The Kotlin layer pre-checks with nativeCountTokens and raises ContextTooLarge
    // instead — this path should only fire if that check was bypassed.
    ...
    // Conservative sampler tuned for single-action JSON decisions on small
    // instruct models: low temperature, mild top-p. Caller-requested temperature
    // is honored when provided (>= 0), clamped to a sane mobile band.
    auto *smpl = llama_sampler_chain_init(llama_sampler_chain_default_params());
    llama_sampler_chain_add(smpl, llama_sampler_init_top_p(0.92f, 1));
    const float temp = (temperature >= 0.0f) ? std::min(std::max(temperature, 0.05f), 1.5f) : 0.25f;
    ...
    // Prompt in chunks well under n_batch: keeps Cancel responsive (g_cancel is
    // only checked between chunks) and gives granular prefill progress.
    int pos = 0;
    while (pos < n_prompt) {
        if (g_cancel) break;
    ...
    if (!g_cancel) {
        ...
        for (int i = 0; i < max_tokens; i++) {
            ...
            // boundaries, so re-scan only a small tail window (any match that
